...
namespace cftf
{
    ...
    public:
        ...
        {
            if (specialized_stmt->getBeginLoc() == stmt->getBeginLoc())
            {
                match = stmt;
    ...
    bool ASTVisitor::VisitIfStmt(clang::IfStmt* stmt)
    {
        ...
        if (current_function->decl->getPrimaryTemplate())
        {
            ...
            // specialized function. We detect this correspondence based on stmt->getBeginLoc(), which hopefully shouldn't
            // have changed.
        ...
        if (branch)
        {
            ...
            clang::SourceLocation cond_first = stmt->getCond()->getBeginLoc();
            if (stmt->getInit())
            {
                cond_first = stmt->getInit()->getBeginLoc();
            }
            else if (stmt->getConditionVariableDeclStmt())
            {
                cond_first = stmt->getConditionVariableDeclStmt()->getBeginLoc();
            }

            // Don't need to check for initializing statement here since that precedes the condition anyway
            clang::SourceLocation cond_last = stmt->getCond()->getEndLoc();
            if (stmt->getConditionVariableDeclStmt())
            {
                cond_last = stmt->getConditionVariableDeclStmt()->getEndLoc();
            }

            rewriter->ReplaceTextExcludingEndToken({ stmt->getBeginLoc(), cond_first }, "if (");
            rewriter->RemoveTextExcludingEndToken({ getLocForEndOfToken(cond_last), branch->getBeginLoc() });
            if (result)
            {
            ...
            rewriter->RemoveTextIncludingEndToken({ getLocForEndOfToken(branch->getEndLoc()), stmt->getEndLoc() });
        }
        else
        {
            // Condition was false and no else-branch has been given, so just remove the entire statement
            rewriter->RemoveTextIncludingEndToken({ stmt->getBeginLoc(), stmt->getEndLoc() });
        }
