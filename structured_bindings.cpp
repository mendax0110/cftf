...
namespace cftf
{
    ...
    static bool TypeHasStdTupleSizeSpecialization(clang::ASTContext& context, const clang::Type* type)
    {
       ...
       // assert(tuple_size_lookup_result.size() < 2);
        if (tuple_size_lookup_result.empty())
        {
    ...
    bool ASTVisitor::VisitDecompositionDecl(clang::DecompositionDecl* decl)
    {
        ...
        auto rewritten_decl = "/*" + GetClosedStringFor(decl->getBeginLoc(), decl->getEndLoc()) + "*/\n";
        rewritten_decl += "auto " + temp_name + " = " + GetClosedStringFor(decl->getInit()->getBeginLoc(), decl->getInit()->getEndLoc()) + ";\n";
